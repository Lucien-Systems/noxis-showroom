...
function Illustration({id}:{id:string}){
  if(id==="aiml"){
    return (
      <svg viewBox="0 0 320 120" className="w-full h-[120px]">
        <defs><linearGradient id="g1" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#1EE9A6"/><stop offset="100%" stopColor="#FFCC55"/></linearGradient></defs>
        {[0,1,2].map((k)=>(
        ...
        {Array.from({length:12}).map((_,i)=>(
          <circle key={i} cx={i*28} cy={40+(i%3)*10} r="2.5" fill="#1EE9A6" opacity="0.6"/>
        ))}
      </svg>
    );
  }
  if(id==="ingest"){
    return (
      <svg viewBox="0 0 320 120" className="w-full h-[120px]">
        <rect x="12" y="20" width="80" height="22" rx="6" fill="#111A2D" stroke="#1F3046"/>
        <rect x="12" y="52" width="80" height="22" rx="6" fill="#111A2D" stroke="#1F3046"/>
        <rect x="12" y="84" width="80" height="22" rx="6" fill="#111A2D" stroke="#1F3046"/>
        <rect x="210" y="45" width="98" height="30" rx="10" fill="#111A2D" stroke="url(#g2)"/>
        <defs><linearGradient id="g2" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#1EE9A6"/><stop offset="100%" stopColor="#FFCC55"/></linearGradient></defs>
        {[30,62,94].map((y,i)=>(
          <g key={i}><path d={`M92 ${y+11} C 140 ${y+11}, 170 60, 210 60`} stroke="url(#g2)" strokeWidth="3" fill="none"/><circle cx="92" cy={y+11} r="3" fill="#1EE9A6"/></g>
        ))}
      </svg>
    );
  }
  if(id==="receipts"){
    return (
      <svg viewBox="0 0 320 120" className="w-full h-[120px]">
        <rect x="40" y="18" width="160" height="84" rx="10" fill="#111A2D" stroke="#1F3046"/>
        <path d="M60 38 h120" stroke="#1EE9A6" strokeWidth="3"/>
        <path d="M60 58 h100" stroke="#1EE9A6" strokeWidth="3" opacity=".7"/>
        <path d="M60 78 h80" stroke="#FFCC55" strokeWidth="3" opacity=".8"/>
        <rect x="210" y="34" width="70" height="52" rx="8" fill="#111A2D" stroke="url(#g3)"/>
        <defs><linearGradient id="g3" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#FFCC55"/><stop offset="100%" stopColor="#1EE9A6"/></linearGradient></defs>
        <circle cx="245" cy="60" r="18" fill="none" stroke="url(#g3)" strokeWidth="3"/>
        <path d="M238 60 l6 6 l10 -12" fill="none" stroke="#1EE9A6" strokeWidth="3"/>
      </svg>
    );
  }
  if(id==="privacy"){
    return (
      <svg viewBox="0 0 320 120" className="w-full h-[120px]">
        <defs><linearGradient id="g4" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#1EE9A6"/><stop offset="100%" stopColor="#FFCC55"/></linearGradient></defs>
        <rect x="40" y="26" width="240" height="68" rx="14" fill="#111A2D" stroke="#1F3046"/>
        <rect x="70" y="46" width="100" height="28" rx="8" fill="#0B1224" stroke="url(#g4)"/>
        <rect x="190" y="46" width="70" height="28" rx="8" fill="#0B1224" stroke="#1F3046"/>
        <path d="M215 34 v-10 a10 10 0 0 0 -20 0 v10" stroke="#FFCC55" strokeWidth="3" fill="none"/>
      </svg>
    );
  }
  if(id==="stores"){
    return (
      <svg viewBox="0 0 320 120" className="w-full h-[120px]">
        <defs><linearGradient id="g5" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#1EE9A6"/><stop offset="100%" stopColor="#FFCC55"/></linearGradient></defs>
        {Array.from({length:6}).map((_,i)=>(
          <rect key={i} x={30 + (i%3)*90} y={20 + Math.floor(i/3)*50} width="70" height="40" rx="10" fill="#111A2D" stroke={i%2===0?"url(#g5)":"#1F3046"} />
        ))}
        <path d="M65 40 L155 40 L245 40" stroke="#1F3046" strokeDasharray="4 6" />
        <path d="M65 90 L155 90 L245 90" stroke="#1F3046" strokeDasharray="4 6" />
      </svg>
    );
  }
  return (
    <svg viewBox="0 0 320 120" className="w-full h-[120px]">
      <defs><linearGradient id="g6" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#1EE9A6"/><stop offset="100%" stopColor="#FFCC55"/></linearGradient></defs>
      {Array.from({length:7}).map((_,i)=>(
        <rect key={i} x={24+i*36} y={20+(i%3)*10} width="26" height={60-(i%3)*12} rx="6" fill="url(#g6)" opacity={0.5+(i%3)*0.15}/>
      ))}
      <circle cx="250" cy="40" r="20" fill="none" stroke="#1F3046"/>
      <circle cx="250" cy="40" r="8" fill="#1EE9A6" opacity=".8"/>
      <path d="M250 60 L270 90" stroke="#FFCC55"/>
      <circle cx="270" cy="90" r="6" fill="#FFCC55"/>
    </svg>
  );
}
    ...
    <div className="card p-6">
      <div className="grid gap-4 md:grid-cols-[300px_1fr]">
        ...
        <div className="relative overflow-hidden card p-6">
          <div className="absolute inset-0 bg-[radial-gradient(700px_260px_at_-120px_-160px,rgba(255,204,85,.12),transparent_70%)]" />
